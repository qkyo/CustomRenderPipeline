...
// If we have a point light then we must use the appropriate axis-aligned plane instead.
// Use a light plane that matches the face orientation.
...
// @param bounds: use it to clamp the position in shadow tile space.
...
float GetOtherShadow (OtherShadowData other, ShadowData global, Surface surfaceWS) 
{
	...
	// If we have a point light then we must use the appropriate axis-aligned plane instead.
	// The order of the cubemap faces is +X, −X, +Y, −Y, +Z, −Z,
	// which matches how we rendered them. Add the offset to the tile index.
	if (other.isPoint) {
		// find the face offset
	...
	float distanceToLightPlane = dot(surfaceToLight, lightPlane);		// distance from light to plane
	// use normal bias to scale the normal according the distance from light
...
float GetOtherShadowAttenuation (OtherShadowData other, ShadowData global, Surface surfaceWS) 
{
	...
	float shadow;
	// The global strength is used to determine whether we can skip sampling realtime shadows, 
	// either because we're beyond the shadow distance or outside the largest cascade sphere.
