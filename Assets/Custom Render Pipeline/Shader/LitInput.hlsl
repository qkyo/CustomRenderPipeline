...
float4 GetDetail (InputConfig c) {
	if (c.useDetail) {
		...
		return map * 2.0 - 1.0;						// Converting the detail value range from (0,1) to (−1,1)
	}
	return 0.0;
}
...
float4 GetBase (InputConfig c) {
	...
	if (c.useDetail) {
		...
		map.rgb = lerp(sqrt(map.rgb), detail < 0.0 ? 0.0 : 1.0, abs(detail) * mask);			// Interpolating the color with either 0 or 1
																						// sqrt(map.rgb) is for applying the modification in gamma space
																						// So that the detail would better match a visually equal distribution
		map.rgb *= map.rgb;
	}
...
float GetSmoothness (InputConfig c) {
	...
	if (c.useDetail) {
		...
		float mask = GetMask(c).b;		// Detail channel
